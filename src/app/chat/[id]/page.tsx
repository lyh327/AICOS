...
export default function ChatPage() {
  ...
  // 追踪当前处理模式状态
  const [currentMode, setCurrentMode] = useState<'standard' | 'smart'>('smart');
  ...
  const handleSendMessage = async () => {
        ...
        if (segments.length > 1) {
          ...
          if (segmentConfirm) {
            ...
            for (let i = 0; i < segments.length; i++) {
              ...
              await handleSingleMessage(segmentMessage, currentMode, true);
    ...
    // 单条消息发送
    setInputMessage('');
    await handleSingleMessage(currentInput, currentMode, true);
  };
